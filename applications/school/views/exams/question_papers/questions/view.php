...
<div class="col-md-12 innerview">
	...
	<form method="POST" action="<?php echo site_url('question/create'); ?>">
...
<script type="text/javascript">
	$(document).ready(function () {
		$('#table-view').load('<?php echo site_url('question/display') ?>')
	});

	$('.filter').on('change', function () {
		if ($('#filter-check').is(':checked')) {
			$('#table-view').load('<?php echo site_url('question/filter/') ?>' + $('#year').val() + '/' + $('#month').val() + '/' + $('#class').val())
		}
	});
