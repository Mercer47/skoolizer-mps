...
<div class="col-md-12 innerview">
	...
	<form method="POST" action="<?php echo site_url('questionPaper/create'); ?>">
...
<script type="text/javascript">
	$(document).ready(function () {
		$('#table-view').load('<?php echo site_url('questionPaper/display') ?>')
	});

	$('.filter').on('change', function () {
		if ($('#filter-check').is(':checked')) {
			$('#table-view').load('<?php echo site_url('questionPaper/filter/') ?>' + $('#year').val() + '/' + $('#month').val() + '/' + $('#class').val())
		}
	});
