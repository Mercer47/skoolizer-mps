...
<div class="col-md-12 innerview">
	...
	<form method="POST" action="<?php echo site_url('assignment/create'); ?>">
...
<script type="text/javascript">
	$(document).ready(function () {
		$('#table-view').load('<?php echo site_url('assignment/display') ?>')
	});

	$('.filter').on('change', function () {
		if ($('#filter-check').is(':checked')) {
			$('#table-view').load('<?php echo site_url('assignment/filter/') ?>' + $('#year').val() + '/' + $('#month').val() + '/' + $('#class').val())
		}
	});
