	...
	<?php foreach($messages as $row) { ?>
		<div class="col-xs-12" style="margin-bottom: 20px;">
			<div class="col-xs-2" style="padding: 0px; padding-top: 10px;">
				<?php if($row->audience=='Teachers') { ?>
					<img src="<?php echo base_url('assets/icons/teacher.svg'); ?>" class="msgicon">
				<?php } else if ($row->audience=='Librarian') { ?>
				<img src="<?php echo base_url('assets/icons/librarysms.svg'); ?>" class="msgicon">
				<?php } else if ($row->audience=='Everyone') { ?>
					<img src="<?php echo base_url('assets/icons/teamwork.svg'); ?>" class="msgicon">
				<?php } else if ($row->audience=='Students') { ?>
					<img src="<?php echo base_url('assets/icons/student.svg'); ?>" class="msgicon">
				<?php } else if ($row->audience=='Transport Staff') { ?>
					<img src="<?php echo base_url('assets/icons/school-bus.svg'); ?>" class="msgicon">
			<?php	} else if ($row->audience=='Office Staff') { ?>
				<img src="<?php echo base_url('assets/icons/man.svg'); ?>" class="msgicon">
			<?php } ?>
		
			</div>
		<div class="col-xs-10" style="border-bottom: 1px solid; border-color: #515151;">
			<p style="text-transform: uppercase; font-family: Nunito-Semibold; font-size: 20px;"><?php echo $row->audience; ?><span style="float: right; font-size: 10px;"><?php 
...
<a href="<?php echo site_url('Principal/newmessage') ?>" class="float">
<i class="material-icons" style="font-size: 30px; color: white; position: relative;top: 18px;">
message
</i>
</a>
<?php } else { ?>
	...
	<div class="col-xs-12" align="center" style="padding-top: 80px;">
		...
		<button class="btn" onclick="location.href='<?php echo site_url('Principal/newmessage') ?>'">Create New Message</button>
	</div>
<?php } ?>
